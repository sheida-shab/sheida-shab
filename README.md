## Hi there 👋,I'm Sheida

An experienced Data Analyst and Software Engineer retraining through **Code Your Future (CYF)**. I completed an intensive year-long software development program, strengthening my skills in **Python, SQL, JavaScript, Git, Unix/Linux, and modern software engineering practices**. Now focused on building reliable, high-performance systems and applying my background in data management, reporting, and optimisation to real-world software projects.

---

## 📫 Reach Me at

- **Email:** shabankary@gmail.com 
- **LinkedIn:** https://www.linkedin.com/in/sheida-shabankari/  
- **Portfolio:** https://github.com/sheida-shab
- **Codewars:** https://www.codewars.com/users/sheida-shab

---

## 🛠 Skills

- **Languages:** Python, SQL, JavaScript, Delphi  
- **Data & Tools:** ETL, KPIs, Reporting, Excel, Airtable  
- **Software Engineering:** Git, GitHub, Unix/Linux, CS fundamentals  
- **Other:** Data cleaning, automation, process optimisation

---

## 💼 Experience Highlights

- **Software:** Developed desktop applications for insurance systems;
- **Data Projects:** Designed and mintained multi-million record  operational databases ; .  
- **Volunteer Work:** automated manual workflows , improved accuracy, and implemented dashboards & automation.

---

## 📂 Projects

- https://github.com/sheida-shab/Simple-Chat-Application-v2.git
- https://github.com/sheida-shab/Days-calendar-project.git
- https://github.com/sheida-shab/insurance-loss-ratio-sql.git  
- https://github.com/sheida-shab/Project-TV-Show.git

---



> ⚡ I’m passionate about building reliable, high-performance systems and applying my data and software skills to real-world problems.
